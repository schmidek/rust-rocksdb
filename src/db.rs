...
impl<I: DBInner> DBCommon<SingleThreaded, I> {
    ...
    pub fn drop_cfs(&mut self, names: &[&str]) -> Result<(), Error> {
        let cfs = names.iter().map(|name| self.cfs.cfs.remove(*name).ok_or_else(|| Error::new(format!("Invalid column family: {}", *name)))).collect::<Result<Vec<_>, Error>>()?;
...
impl<I: DBInner> DBCommon<MultiThreaded, I> {
    ...
    pub fn drop_cfs(&self, names: &[&str]) -> Result<(), Error> {
        let cfs = names.iter().map(|name| self.cfs.cfs.write().unwrap().remove(*name).ok_or_else(|| Error::new(format!("Invalid column family: {}", *name)))).collect::<Result<Vec<_>, Error>>()?;
        let mut cf_inners: Vec<_> = cfs.iter().map(|cf| cf.inner).collect();
        self.drop_column_families(&mut cf_inners[..], cfs)
    }
